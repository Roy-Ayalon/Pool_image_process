...
def detect_stick(image, mask):
    def convert_to_cmyk_k_channel(image, mask_2):
        ...
        image_proc = image_proc * mask_2 + replacement_color * (mask_2)
    ...
    def detect_and_refine_stick(image, binary_k_channel):
        ...
        start_point = (start_point_1, start_point_2)
        end_point = (end_point_1, end_point_2)

        # Draw the refined line on the original image
        if start_point and end_point:
            cv2.line(result_img, start_point[0], end_point[0], (0, 0, 255), thickness=3)  # Blue
            cv2.line(result_img, start_point[1], end_point[1], (0, 0, 255), thickness=3)
        else:
