...
def get_stick_tip_points(frame, board_mask):
    ...
    # Convert the frame to HSV and threshold it to get tip_mask.
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    tip_mask = cv2.inRange(hsv, static_lower, static_upper)

    # Ensure board_mask has the same shape as tip_mask.
    if board_mask.shape != tip_mask.shape:
        board_mask = cv2.resize(board_mask, (tip_mask.shape[1], tip_mask.shape[0]),
                                interpolation=cv2.INTER_NEAREST)

    # Convert board_mask to uint8 if it isn’t already.
    if board_mask.dtype != np.uint8:
        board_mask = board_mask.astype(np.uint8)

    # Mask the tip area by the board region.
    tip_mask_board = cv2.bitwise_and(tip_mask, tip_mask, mask=board_mask)

    # Find the largest red cluster inside the board area.
    largest_cluster = find_largest_red_cluster(tip_mask_board)
    if largest_cluster is None:
        return None, None

    # Fit a line to this cluster (the pool cue tip).
    pt1, pt2 = fit_stick_line(largest_cluster)
    return pt1, pt2