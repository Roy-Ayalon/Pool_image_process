...
from detect_balls import detect_white_ball
...
def trajectory(frame, line, board_contour, balls_info):
    """
    Plot the trajectory and determine the first object the line intersects.
    ...
    okay = okay_to_shoot(frame, line)
    if okay:
        _, white_ball = detect_white_ball(frame, board_contour)
        x, y, r = white_ball
        white_center = (x,y)
        white_radius = r
        plot_line(frame, white_center, white_radius, line, board_contour, dash_length=20, color=(255, 255, 0), thickness=2)
    else:
        return